...
function finity.new(themee, gprojectName, thinProject)
	local finityObject = {}
		...
		finityData.UpConnection = finity.gs["UserInputService"].InputEnded:Connect(function(Input, typing)
			if Input.KeyCode == finityData.ToggleKey and not typing then
                ...
                if toggled then
					...
					--pcall(self2.container.TweenPosition, self2.container, savedposition, "Out", "Sine", 10, true)
                else
			...
			function sector:DarkStorm(kind, name, callback, data)
				...
				if kind then
					if string.lower(kind) == "checkbox" or string.lower(kind) == "toggle" then
						...
						if data then
							if data.enabled then
								finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_checked}):Play()
								finity.gs["TweenService"]:Create(cheat.checkboxbutton, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_checked}):Play()
							end
						end
						...
						cheat.checkboxbutton.MouseEnter:Connect(function()
							...
							finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(0.1), {ImageColor3 = lightertheme}):Play()
						end)
						cheat.checkboxbutton.MouseLeave:Connect(function()
							if not cheat.value then
								finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_outer}):Play()
							else
								finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_checked}):Play()
							end
						end)
						cheat.checkboxbutton.MouseButton1Down:Connect(function()
							if cheat.value then
								finity.gs["TweenService"]:Create(cheat.checkboxbutton, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_outer}):Play()
							else
								finity.gs["TweenService"]:Create(cheat.checkboxbutton, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_checked}):Play()
							end
						end)
						cheat.checkboxbutton.MouseButton1Up:Connect(function()
							...
							if cheat.value then
								...
								finity.gs["TweenService"]:Create(cheat.outerbox, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_outer}):Play()
								finity.gs["TweenService"]:Create(cheat.checkboxbutton, TweenInfo.new(0.1), {ImageColor3 = theme.checkbox_inner}):Play()
							end
						end)
                    ...
                    elseif string.lower(kind) == "dropdown" then
						...
						if data.multiselect and data.multiselect then
				        	cheat.value = {}
						else
							cheat.value = nil
				        end
						...
						function cheat.refreshOptions()
								...
								button.MouseButton1Click:Connect(function()
									if cheat.dropped then
										if data.multiselect and data.multiselect then
											...
											if cheat.value[1] then
												if data.placeholder[2] then
													cheat.selected.Text = "Selected " .. data.placeholder[2] .. ": " .. #cheat.value
												else
													cheat.selected.Text = "Selected Options: " .. #cheat.value
												end
											else
												cheat.selected.Text = data.placeholder[1]
											end
										else
											cheat.value = value
							...
							--finity.gs["TweenService"]:Create(cheat.list, TweenInfo.new(0), {Size = UDim2.new(1, 0, 0, 0), Position = UDim2.new(0, 0, 1, 0), CanvasSize = UDim2.new(0, 0, 0, uilistlayout.AbsoluteContentSize.Y), ScrollBarImageTransparency = 1, BackgroundTransparency = 1}):Play()
						end
							...
							if cheat.dropped then
								for _, button in next, cheat.list:GetChildren() do
									if button:IsA("TextButton") then
										if data.multiselect and data.multiselect and table.find(cheat.value, button.Text) then
											finity.gs["TweenService"]:Create(button, TweenInfo.new(0), {TextColor3 = Color3.fromRGB(132,255,130), TextTransparency = 0}):Play()
										else
											finity.gs["TweenService"]:Create(button, TweenInfo.new(0), {TextColor3 = theme.dropdown_text, TextTransparency = 0}):Play()
										end
									end
								end
								--finity.gs["TweenService"]:Create(cheat.list, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, math.clamp(uilistlayout.AbsoluteContentSize.Y, 0, 150)), Position = UDim2.new(0, 0, 1, 0), ScrollBarImageTransparency = 0, BackgroundTransparency = 0.5}):Play()
							else
								for _, button in next, cheat.list:GetChildren() do
									if button:IsA("TextButton") then
					...
					elseif string.lower(kind) == "keybind" or string.lower(kind) == "bind" then
						...
						cheat.button.MouseButton1Up:Connect(function()
							...
							connection = finity.gs["UserInputService"].InputBegan:Connect(function(Input)
								...
								elseif Input.KeyCode == Enum.KeyCode.Backspace then
									callback_bind = nil
									cheat.button.Text = data.placeholder[1]
									cheat.value = nil
									cheat.holding = false
									delay(0, function()
										if callback then
                        ...
                        function cheat:SetValue(value)
                            ...
                            cheat.button.Text = tostring(value)
							callback_bind = nil
							cheat.holding = false
							if callback then
								local s, e = pcall(function()
